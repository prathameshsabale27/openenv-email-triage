...
The OpenEnv server adapter exposes the same task logic through the standard FastAPI runtime expected by `openenv validate` and Hugging Face Spaces.
...
Each task includes a deterministic programmatic grader in [email_triage_env/grader.py](/P:/openenv-email-triage/email_triage_env/grader.py). The grader returns a score in `[0.0, 1.0]` based on weighted completion of the required fields.
